										...
										<div class="widget-main">
											...
											<div class="row">
												...
												<div class="col-sm-6">
													...
													<center>
														<a href="<?= site_url('staff/temuan'); ?>" class="btn btn-default"><i class="fa fa-arrow-left"></i> Kembali </a>

													<?php if(!$tl) { ?>
														<a href="<?= site_url('staff/temuan/tambah_tl/'. $temuan->id_temuan .' '); ?>" class="btn btn-primary"> Buat Penugasan Tindak Lanjut </a>
													<?php } ?>
													</center>
												</div>
											</div>
...
<script type="text/javascript">
	$('#modal-kat').on('show.bs.modal', function(e)
	{
		...
		$.ajax({
			type : 'post',
			url  : "<?= site_url('staff/temuan/get_status'); ?>",
			data : {id1:id1},
...
<script type="text/javascript">
	$('#modal-upload').on('show.bs.modal', function(e)
	{
		...
		$.ajax({
			type : 'post',
			url  : "<?= site_url('staff/temuan/get_upload'); ?>",
			data : {id1:id1},
			success : function(data)
			{
				$('.isi-data').html(data);
