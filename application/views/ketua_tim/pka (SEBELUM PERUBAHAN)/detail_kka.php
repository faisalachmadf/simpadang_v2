<!--
  ## DETAIL KKA (KETUA TIM) ##
  Ditampilkan dalam modal bootstrap
-->

<form class='form-horizontal' role='form' action="<?= site_url('ketua_tim/pka/persetujuan_kka/'.base64_encode($kka->sub_pka3).'/'.base64_encode($kka->sub_no_kka).'/'.base64_encode($kka->pelaksana)); ?>" method='post'>
  ...
  <table width="100%" border="1">
    <tr>
      ...
      <td width="28%" class="c b bg-color"> Reviu DALNIS </td>
      <td class="c b bg-color"> Reviu DALTU </td>
    ...
    <?php
      foreach($data_rev as $row)
      {
        ...
        <tr>
          ...
          <td class='pos-atas'>";
            if($row->rev_kka_dalnis == "-")
              { echo "<span class='green'><i class='fa fa-check'></i> Tidak ada reviu </span>"; }
            else
              { echo "<span class='red'> $row->rev_kka_dalnis </span>"; }
          echo "
          </td>
          <td class='pos-atas'>";
            if($row->rev_kka_daltu == "-")
              { echo "<span class='green'><i class='fa fa-check'></i> Tidak ada reviu </span>"; }
            else
              { echo "<span class='red'> $row->rev_kka_daltu </span>"; }
          echo "
          </td>
          <td class='c pos-atas'>";
            if($row->rev_bukti_kka != NULL || $row->rev_bukti_kka != "")
            {
              ...
              <div class='btn-group'>
                ...
                <ul class='dropdown-menu dropdown-primary'>
                  <li>
                    <a href='".site_url('ketua_tim/pka/download_rev_kka/'.base64_encode($row->rev_hasil_kka))."' title='Download KKA reviu'> Kertas Kerja Audit (KKA)</a>
                  </li>

                  <li>
                    <a href='".site_url('ketua_tim/pka/download_bukti_kka/'.base64_encode($row->rev_bukti_kka))."' title='Download Bukti KKA'> Bukti Pendukung KKA</a>
                  </li>
                </ul>
              </div>";
            }
            ...
            {
              echo "<a href='".site_url('ketua_tim/pka/download_rev_kka/'.base64_encode($row->rev_hasil_kka))."' class='btn btn-sm btn-primary' title='Download KKA reviu'><i class='fa fa-download'></i></a>";
            }
          echo "
          </td>
        </tr>
  ...
  <div class="row">
    <div class="col-sm-12">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-8">
          <a href="<?= site_url('ketua_tim/pka/download_kka/'.base64_encode($kka->hasil_kka)); ?>" class='btn btn-sm btn-success' title='Download kertas kerja audit'><i class='fa fa-download'></i> KKA </a>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="col-sm-8">
          <a href="<?= site_url('ketua_tim/pka/download_bukti_kka/'.base64_encode($kka->bukti_kka)); ?>" class='btn btn-sm btn-danger' title='Download bukti pendukung KKA'><i class='fa fa-download'></i> Bukti Pendukung KKA </a>
        </div>
      </div>
      ...
      <table width="100%" border="0">
        <tr>
          <td width="17%" class="pos-atas"> Reviu Ketua Tim </td>
          <td width="2%" class="pos-atas"> : </td>
          <td>
            <?php
              if($kka->reviu_kka_ketua == NULL)
