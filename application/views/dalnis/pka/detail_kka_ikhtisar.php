<!--
  ## DETAIL KKA IKHTISAR (DALNIS) ##
  Ditampilkan dalam modal bootstrap
-->

<form class='form-horizontal' role='form' action="<?= site_url('dalnis/pka/persetujuan_kka_ikhtisar/'.base64_encode($kka->sub_pka2).'/'.base64_encode($kka->no_kka)); ?>" method='post'>
    ...
    <?php
      foreach($data_rev as $row)
      {
        ...
        <tr>
          ...
          <td class='c pos-atas'>";            
            echo "<a href='".site_url('dalnis/pka/download_rev_kka_ikhtisar/'.base64_encode($row->rev_kka_ikhtisar))."' class='btn btn-sm btn-primary' title='Download KKA Ikhtisar reviu'><i class='fa fa-download'></i></a>";            
          echo "
          </td>
        </tr>
        ";
  ...
  <div class="row">
    <div class="col-sm-12">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-8">
          <a href="<?= site_url('dalnis/pka/download_kka_ikhtisar/'.base64_encode($kka->kka_ikhtisar)); ?>" class='btn btn-sm btn-success' title='Download kertas kerja audit'><i class='fa fa-download'></i> KKA Ikhtisar </a>
        </div>
      </div>
