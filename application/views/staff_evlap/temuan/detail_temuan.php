										...
										<div class="widget-main">
											...
											<div class="row">
												<div class="col-sm-6">
													...
													<h3 class="header">
														<i class="fa fa-file-text"></i> Usulan Tindak Lanjut
													</h3>
													<div class="well">
														<?= $temuan->isi_usulan; ?>
													</div>
												...
												<div class="col-sm-6">
													...
													<center>
														<a href="<?= site_url('staff_evlap/temuan'); ?>" class="btn btn-default"><i class="fa fa-arrow-left"></i> Kembali </a>
														<a target="_blank" href="<?= site_url('staff_evlap/temuan/cetak_temuan/'. $temuan->id_temuan .' '); ?>" class="btn btn-primary"><i class="fa fa-print"></i> Cetak Temuan </a>
													</center>
												</div>
											</div>
							...
							foreach ($temuan_rekomendasi as $rek_spi)
							{
								...
								if($rek_spi->aspek == "spi" && $spi->id_temuan_aspek == $rek_spi->id_temuan_aspek)
								{
									...
									<td class='c'>
										...
										<a href='#modal-kat' role='button' class='' data-toggle='modal'  data-id1='".$rek_spi->id_temuan_rekomendasi."' title='Tentukan kategori'> Lihat </a>
						...
						if($e3->aspek == "e3")
						{
							...
							foreach ($temuan_rekomendasi as $rek_e3)
							{
								...
								if($rek_e3->aspek == "e3" && $e3->id_temuan_aspek == $rek_e3->id_temuan_aspek)
								{
									...
									<td class='c'>
										$e3_status_tl <br><br>
										<a href='#modal-kat' role='button' class='' data-toggle='modal'  data-id1='".$rek_e3->id_temuan_rekomendasi."' title='Tentukan kategori'> Lihat </a>
									</td>
							...
							foreach ($temuan_rekomendasi as $rek_kepatuhan)
							{
								...
								if($rek_kepatuhan->aspek == "kepatuhan" && $kepatuhan->id_temuan_aspek == $rek_kepatuhan->id_temuan_aspek)
								{
									...
									<td class='c'>
										...
										<a href='#modal-kat' role='button' class='' data-toggle='modal'  data-id1='".$rek_kepatuhan->id_temuan_rekomendasi."' title='Tentukan kategori'> Lihat </a>
...
<script type="text/javascript">
	$('#modal-kat').on('show.bs.modal', function(e)
	{
		...
		$.ajax({
			type : 'post',
			url  : "<?= site_url('staff_evlap/temuan/get_status'); ?>",
			data : {id1:id1},
...
<script type="text/javascript">
	$('#modal-upload').on('show.bs.modal', function(e)
	{
		...
		$.ajax({
			type : 'post',
			url  : "<?= site_url('staff_evlap/temuan/get_upload'); ?>",
			data : {id1:id1},
			success : function(data)
			{
				$('.isi-data').html(data);
