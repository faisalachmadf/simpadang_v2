...
jQuery(document).ready(function(){
	"use strict";
	Axiom_Swipers = {};
	if(Axiom_css_animation) setTimeout(Axiom_animation_shortcodes, 600);
});
...
function initShortcodes(container) {
	...
	// Contact form
	if (container.find('.sc_contact_form a.sc_contact_form_submit:not(.inited)').length > 0) {
		container.find(".sc_contact_form a.sc_contact_form_submit:not(.inited)")
			.addClass('inited')
			.click(function(e){
				var form = jQuery(this).parents("form");
				var action = form.attr('action');
				userSubmitForm(form, action!=undefined ? action : Axiom_ajax_url, Axiom_ajax_nonce);
				e.preventDefault();
				return false;
			});
	}
		...
		container.find('.sc_slider_swiper:not(.inited)')
			.each(function () {
				...
				if(jQuery(this).parent().hasClass('sc_text_slider') || jQuery(this).parent().hasClass('sc_testimonials') || jQuery(this).parent().hasClass('sc_twitter') ) {
					Axiom_Swipers[id] = new Swiper('.' + id, {
						loop: true,
						grabCursor: true,
					...
					jQuery(window).resize(function(){
						Axiom_Swipers[id].resizeFix();
						Axiom_Swipers[id].reInit();
					});
				...
				if (curSlide > 0)
					Axiom_Swipers[id].swipeTo(curSlide-1);
...
function prepareSliderNavi(slider) {
	...
	if (navi.length > 0) {
		navi.find('li').click(function(e){
			...
			var id = swiper.attr('id');
			Axiom_Swipers[id].swipeTo(jQuery(this).index());
			e.preventDefault();
			return false;
		});
	}
	...
	container.find('.sc_skills_arc:not(.inited)').each(function () {
		...
		var o = {
			...
			diagram: function(){
				...
				r.circle(c, c, Math.round(w/2)).attr({ stroke: 'none', fill: Axiom_theme_skin_bg ? Axiom_theme_skin_bg : '#ffffff' });
				
				var title = r.text(c, c, arc.data('subtitle')).attr({
					font: 'lighter '+Math.round(rad*0.8)+'px "'+Axiom_theme_font+'"',
					fill: '#888888'
				}).toFront();
